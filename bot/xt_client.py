...
class XTClient:
    ...
    Notes for CROSS (CROSSED) margin trading with high leverage / low margin:
    - Position type value in the XT API is "CROSSED" (NOT "CROSS"); "ISOLATED"
      is the other value.
    - adjust_margin() and set_auto_margin() are ISOLATED-position features only.
      In CROSSED mode there is no per-position margin to add, and "auto margin"
      does not exist (the whole available balance backs the position already).
      Do NOT call them when trading CROSSED — they will error or do nothing.
    - In CROSSED mode the only fix for an "insufficient margin/balance" error is
      to size the order down from availableBalance (see get_balances()).
    """
    ...
    def _sign_headers(self, path: str, params: dict = None):
        ...
        # against the current official XT docs (doc.xt.com - SignatureStatement
        # and every curl example), which use validate-appkey / validate-timestamp
        # / validate-signature / validate-recvwindow / validate-algorithms.
        # Repos like AItradekit / xt-exchange-plugin still use "xt-validate-"
        # (the OLD contract, also what CCXT mirrors) — that makes every signed
        # call fail on the current USDT-M API.
        ...
        return {
            ...
            # Docs recommend <= 5000ms; 60000 may be rejected as out of range.
        ...
        for attempt in range(max_retries):
            try:
                ...
                if method == "GET":
                    ...
                    # POST: docs put params in the form-encoded BODY, not the
                    # query string. Signature string is unchanged (same dict).
    ...
    def get_listen_key(self) -> str:
        # Valid for 8 hours (server-side) — re-call this method before expiry
        # if you rely on the user-data WebSocket stream.
    ...
    def set_position_type(self, symbol: str, position_side: str, position_type: str):
        ...
        # accepted here and normalized so old callers keep working.
    ...
    def set_auto_margin(self, symbol: str, position_side: str, enabled: bool):
        ...
        # (the whole available balance already backs the position) — do NOT call
        # this when trading CROSSED; it will error or be a no-op.
    ...
    def create_order(self, symbol: str, position_side: str, order_side: str,
        ...
        # IMPORTANT for CROSSED + low margin: always close/reduce positions with
        # reduce_only=True so the order can never open a new position.
    ...
    def create_tpsl(self, symbol: str, position_side: str, orig_qty: int,
        ...
        # it defaults to year 2100 -> effectively NO time limit. Pass your own
        # ms timestamp only if you really want the TP/SL to expire.
